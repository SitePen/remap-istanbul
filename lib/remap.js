			...
			Object.keys(item).forEach(function (filePath) {
				...
				if (sourceMap.sourcesContent) {
					sourceMap.sourcesContent.forEach(function (source, idx) {
						...
						getSourceCoverage(srcCoverage, sourceMap.sources[idx]).data.code = source;
						if (sourceStore) {
				...
				Object.keys(fileCoverage.statementMap).forEach(function (index) {
					...
					// Skip items where they're flagged for skipping and
					// have no corresponding source line.
					if (genItem.skip && genItem.start.line === 0) {
  						return;
  					}
